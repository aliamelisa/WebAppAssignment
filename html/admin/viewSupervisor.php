...
<html>
    ...
    <div id="table-place">
    <h2 style="text-align:center;">Supervisor Information</h2>
    <form action="deleteSV.php" method="POST">
        <table id='infotable'>
            <th></th>
            <th>Supervisor ID</th>
            <th>Name</th>
        ...
        $sql = "SELECT supervisorID, fullName, password, phoneNumber, email FROM supervisor";
        $result = mysqli_query($conn, $sql);

        if ($result->num_rows > 0) {
            while($row = mysqli_fetch_assoc($result)) {
                echo "<tr>
                        <td><input type='checkbox' name='selectUser[]' value='" . $row["supervisorID"] . "'></td>
        ...
        <p></p>
        <button type="button" onclick="window.location.href='newSV.php';">Add New User</button>
        <p></p>
        <button type='button' onclick="updateSV();">Update Information</button>
        <p></p>
