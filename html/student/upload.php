    ...
    if (isset($_FILES["project-file"]) && $_FILES["project-file"]["error"] == 0) {
        ...
        if (!in_array($file_type, $allowed_types)) {
            ...
            if (move_uploaded_file($_FILES["project-file"]["tmp_name"], $target_file)) {
                ...
                $sql = "INSERT INTO proposal (supervisorID, studentID, specialization, projectTitle, projectType, 
                filename, proposalstatus) VALUES ('$supervisorID', '$studentID', '$specialisation', '$projecttitle', 
                '$projecttype', '$filename', 'pending')";

                if ($conn->query($sql) === TRUE) {
                    echo "<script>alert('Upload successful!'); window.location.href = 'studentMain.php';</script>";
                } else {
